...
mcp = FastMCP(
    "Odoo MCP Server",
    instructions="Provides tools and resources to interact with an Odoo ERP system.",
...
def odoo_agent_resource(
    ...
    try:
        ...
        if model and not fields:
            ...
            return json.dumps(
                {"step": "choose_fields", "fields": model_fields}, indent=2
            )
        if model and fields:
            ...
            results = odoo_client.search_read(
                model, domain_list, fields=field_list, limit=limit
            )
...
def execute_method(
    ...
    Parameters:
        ...
        args: Positional arguments as JSON string (e.g., '["arg1", "arg2"]')
        kwargs: Keyword arguments as JSON string (e.g., '{"key": "value"}')
    ...
    try:
        ...
        if method in search_methods and parsed_args:
            ...
            if len(normalized_args) > 0:
                ...
                parsed_args = normalized_args

                # Log for debugging
                print(f"Executing {method} with normalized domain: {domain_list}")

        result = odoo.execute_method(model, method, *parsed_args, **parsed_kwargs)
